...
Status RtreeSecIndexReader::Create(
  ...
  index_reader->reset(new RtreeSecIndexReader(table, std::move(index_block)));
  ...
  RtreeSecIndexReader* const rtree_index_reader =
        static_cast<RtreeSecIndexReader*>(index_reader->get());
    rtree_index_reader->rtree_height_ = rtree_height;
    rtree_index_reader->meta_index_iterator_ = meta_index_iter;
...
InternalIteratorBase<IndexValue>* RtreeSecIndexReader::NewIterator(
...
Status RtreeSecIndexReader::CacheDependencies(const ReadOptions& ro,
                                               bool pin) {
