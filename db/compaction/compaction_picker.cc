...
// GetMbrList
void CompactionPicker::GetMbrList(const CompactionInputFiles& inputs,
...
// Special expandInputsToCleanCut for spatial compaction
bool CompactionPicker::CheckingExpandInputsToCleanCutMbr(const std::string& /*cf_name*/,
...
bool CompactionPicker::IsRangeInCompactionMbr(VersionStorageInfo* vstorage,
                                           const std::vector<Mbr>* mbr_vect,
                                           int level, ImmutableOptions ioptions) {
  std::vector<FileMetaData*> inputs;
  assert(level < NumberLevels());

  vstorage->GetkMaxOverlappingInputs(level, mbr_vect, &inputs, ioptions, -1);
  return AreFilesInCompaction(inputs);
}
...
bool CompactionPicker::SetupOtherInputs(
    ...
    int base_index, ImmutableOptions ioptions, bool only_expand_towards_right) {
  assert(!inputs->empty());
  ...
  //                                     &output_level_inputs->files, ioptions, 2);
  ...
  (void) parent_index;
  (void) base_index;
  (void) only_expand_towards_right;
...
Compaction* CompactionPicker::CompactRange(
  ...
  if (input_level != output_level) {
    ...
    if (!SetupOtherInputs(cf_name, mutable_cf_options, vstorage, &inputs,
                          &output_level_inputs, &parent_index, -1, ioptions_)) {
      // manual compaction is now multi-threaded, so it can
...
bool CompactionPicker::GetOverlappingL0Files(
  ...
  // =======================================================================================
  // TODO: options.compaction_policy (this one dun change!!!!!)

  // std::vector<Mbr> Mbr_vect;
  // GetMbrList(*start_level_inputs, &Mbr_vect);
  // start_level_inputs->files.clear();
  // vstorage->GetkMaxOverlappingInputs(0, &Mbr_vect,
  //                                     &(start_level_inputs->files), ioptions, -1);

  // GetMbrList(*start_level_inputs, &Mbr_vect);
  // if (IsRangeInCompactionMbr(vstorage, &Mbr_vect, output_level, ioptions)) {
  //   return false;
  // }                
