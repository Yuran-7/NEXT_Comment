// Rtree template for memtable;
// Source code is based on https://github.com/nushoin/RTree;
// TODO: This version does not contain a fixed memory allocator as
// other data structure format (i.e., arena);
...
// Fwd decl
class RTFileStream;  // File I/O helper class, look below for implementation and notes.简单的文件读写封装，Save/Load/SaveRec/LoadRec 用其序列化/反序列化整树
...
class RTree // typedef RTree<GlobalSecIndexValue, double, 1, double> GlobalSecRtree;
{
...
public:
  ...
  std::vector<DATATYPE> Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::function<bool (const DATATYPE&)> callback) const;
  ...
  /// Iterator is not remove safe. 使用栈模拟递归向下遍历，支持逐条数据迭代
  class Iterator
  {
  private:
...
protected:
  ...
  struct Rect // rtree.h中也有一个一模一样的Rect，那个是2DR树的矩形结构，这个是RTree模板类的n维矩形结构
  {
  ...
  struct Node
  {
    ...
    int m_count;                                  ///< Count， 是该节点当前“有效分支”的数量，0 <= m_count <= MAXNODES
    int m_level;                                  ///< Leaf is zero, others positive，叶子节点是第0层
    Branch m_branch[MAXNODES];                    ///< Branch
  };

  /// A link list of nodes for reinsertion after a delete operation，删除时“再插入”链表（当某子节点元素过少需回收并重插）
  struct ListNode
  {
...
RTREE_QUAL::RTree()
{
  ...
  m_root = AllocNode(); // new 一个根节点并初始化 m_count=0, m_level=-1
  m_root->m_level = 0;  // 根设为叶层（0 层），表示空树的起始状态
  m_unitSphereVolume = (ELEMTYPEREAL)UNIT_SPHERE_VOLUMES[NUMDIMS];  // 预计算常量
}
...
RTREE_TEMPLATE
void RTREE_QUAL::Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId) // Insert入口
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  InsertRect(branch, &m_root, 0); // 从根开始插入，层级指定为 0（叶子层）
}
...
bool RTREE_QUAL::Load(RTFileStream& a_stream)
{
  ...
  a_stream.Read(dataFileId);  // 读头部信息
  ...
  if(    (dataFileId == _dataFileId)  // 如果校验通过：调用 LoadRec(m_root, a_stream) 递归加载
      && (dataSize == _dataSize)
...
bool RTREE_QUAL::Save(RTFileStream& a_stream)
{
  ...
  a_stream.Write(dataFileId); //写入文件标识 "RTRE"
  a_stream.Write(dataSize); // 写入数据类型大小、维度数、节点容量（TMAXNODES / TMINNODES）
...
{
  a_stream.Write(a_node->m_level);  // 写入节点的层级 m_level 和分支数 m_count
  ...
  if(a_node->IsInternalNode())  // not a leaf node
  {
    for(int index = 0; index < a_node->m_count; ++index)
    {
      ...
      a_stream.WriteArray(curBranch->m_rect.m_min, NUMDIMS);  // 写每个分支的矩形范围（min/max）
      a_stream.WriteArray(curBranch->m_rect.m_max, NUMDIMS);

      SaveRec(curBranch->m_child, a_stream);  // 递归保存子节点
    }
  }
  else // A leaf node
  {
    for(int index = 0; index < a_node->m_count; ++index)
    {
      ...
      a_stream.WriteArray(curBranch->m_rect.m_min, NUMDIMS);  // 写每个分支的矩形范围（min/max）
      a_stream.WriteArray(curBranch->m_rect.m_max, NUMDIMS);  // 写存储的数据 m_data
...
{
  for(int index = 0; index < NUMDIMS; ++index)  // 维度
  {
...
bool RTREE_QUAL::InsertRectRec(const Branch& a_branch, Node* a_node, Node** a_newNode, int a_level)
{
  ...
  if(a_node->m_level > a_level)
  {
    ...
    int index = PickBranch(&a_branch.m_rect, a_node);  // 返回a_node的哪一个branch最合适，branch的下标赋值给index

    // recursively insert this record into the picked branch
    bool childWasSplit = InsertRectRec(a_branch, a_node->m_branch[index].m_child, &otherNode, a_level);

    if (!childWasSplit) // 如果没有分裂
    {
    ...
    else
    {
      ...
      a_node->m_branch[index].m_rect = NodeCover(a_node->m_branch[index].m_child);  // m_child是一个Node
      Branch branch;
      branch.m_child = otherNode; // 被赋值了
  ...
  else if(a_node->m_level == a_level)
  {
    ...
    return AddBranch(&a_branch, a_node, a_newNode); // 可能调用SplitNode，没有分裂返回false
  }
  else
  {
    // Should never occur
...
//
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRect(const Branch& a_branch, Node** a_root, int a_level) // (branch, &m_root, 0)
{
  ASSERT(a_root);
  ...
  if(InsertRectRec(a_branch, *a_root, &newNode, a_level))  // 递归找到合适的叶子，如果子树分裂导致根也要分裂
  {
    // Grow tree taller and new root
    Node* newRoot = AllocNode();  // 新建一个根节点（AllocNode()）
    newRoot->m_level = (*a_root)->m_level + 1;  // 树的高度加一
    ...
    AddBranch(&branch, newRoot, NULL);  // 把旧根和新分裂出来的节点作为两个子节点挂上去
...
RTREE_TEMPLATE
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(const Rect* a_rectA, const Rect* a_rectB) // 注意这里的返回值的语法
{
...
ELEMTYPEREAL RTREE_QUAL::RectSphericalVolume(Rect* a_rect)
{
  ...
  if(NUMDIMS == 3)
  {
    return (radius * radius * radius * m_unitSphereVolume); // 4/3 * pi
  }
  else if(NUMDIMS == 2)
  {
    return (radius * radius * m_unitSphereVolume);  // pi
  }
  else
  {
...
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, std::vector<DATATYPE>& a_return_res, std::function<bool (const DATATYPE&)> callback) const
{
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, a_return_res, callback))  // 递归
        {
          // The callback indicated to stop searching
          return false;
        }
...
RTREE_TEMPLATE
std::vector<typename RTREE_QUAL::Rect> RTREE_QUAL::ListTree() const // RTREE_QUAL::Rect 是依赖于模板参数的“依赖名”（dependent name），没有 typename 会报错：need ‘typename’ before dependent type name
{
  ASSERT(m_root);
  ASSERT(m_root->m_level >= 0);

  std::vector<Rect> treeList; // 它用来遍历整棵 R 树，把所有节点（内部节点与叶子节点）中每个分支的矩形边界(Rect，n 维 AABB)收集成一个列表返回

  std::vector<Node*> toVisit; // 用一个栈 toVisit 做深度优先遍历
